(async function(){try{
 ...
 document.getElementById('back').href='method2.html?outlet='+encodeURIComponent(outlet);document.getElementById('recovery').href='method2-recovery.html?outlet='+encodeURIComponent(outlet);
 function render(s){document.getElementById('rows').replaceChildren();document.getElementById('usageRows').replaceChildren();const usage=new Map();
 let sum=0,packing=0,complete=true;
 ...
 if(c.missing.length)complete=false;else sum+=c.actualDayCost??c.soldCost??0;packing+=c.totalPacking||0;
 ...
 });const shared=M2.orderPackingCost(s);complete=complete&&!shared.missing.length;document.getElementById('status').textContent='Outlet '+outlet+' · '+(complete?'Actual day COGS ₹'+(sum+shared.total).toFixed(2)+' (items ₹'+sum.toFixed(2)+' + shared packing ₹'+shared.total.toFixed(2)+')':'Costs incomplete — review item costs and shared packing.');document.getElementById('packingSummary').textContent='Packing inside item COGS: ₹'+packing.toFixed(2)+' · Shared outlet bags: ₹'+shared.total.toFixed(2)+' · Total packing already included: ₹'+(packing+shared.total).toFixed(2);
 ...
 }render(s);M2OrderUI.init(outlet,s,render);document.getElementById('sharedControls').disabled=false;
