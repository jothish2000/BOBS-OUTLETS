(async function(){try{
 ...
 if(c.missing.length)complete=false;else sum+=c.soldCost||0;packing+=c.totalPacking||0;
 ...
 });const shared=M2.orderPackingCost(s);complete=complete&&!shared.missing.length;document.getElementById('status').textContent='Outlet '+outlet+' · '+(complete?'Total sold COGS ₹'+(sum+shared.total).toFixed(2)+' (items ₹'+sum.toFixed(2)+' + shared packing ₹'+shared.total.toFixed(2)+')':'Costs incomplete — review item costs and shared packing.');document.getElementById('packingSummary').textContent='Packing inside item COGS: ₹'+packing.toFixed(2)+' · Shared outlet bags: ₹'+shared.total.toFixed(2)+' · Total packing already included: ₹'+(packing+shared.total).toFixed(2);
