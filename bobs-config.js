...
window.BOBS_CONFIG = Object.freeze({
  ...
  VERSION: '2026-09-12-111Q-cogs-review-routing'
});
...
(function(){
  ...
  async function saveBatch(input,init){
    ...
    const legacy=String(body.method||'').toLowerCase();const map={method1:'METHOD1',method2:'METHOD2',fixedexpenses:'FIXED_EXPENSES',staffroster:'STAFF_MASTER',hroutletallocation:'HR_OUTLET_ALLOCATION'};const module=map[legacy];
...
document.addEventListener('click',function(e){
  ...
  if(btn){e.preventDefault();e.stopImmediatePropagation();const q=new URLSearchParams(window.location.search);const outletIds=q.get('outlets')||'';const target='cogs-outlet-analysis.html'+(outletIds?'?outlets='+encodeURIComponent(outletIds):'');window.location.href=target;return}
