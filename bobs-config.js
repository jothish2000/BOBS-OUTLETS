...
(function(){
  ...
  function version(){return Date.now()*1000+Math.floor(Math.random()*1000)}
  async function postToVault(payload){const v=payload.stateVersion||version();const data=(payload.data&&typeof payload.data==='object')?payload.data:{};const body=Object.assign({},payload,{stateVersion:v,data:Object.assign({},data,{_bobsMeta:Object.assign({},data._bobsMeta||{},{stateVersion:v,savedBy:'BOBS',savedAt:new Date().toISOString()})})});await nativeFetch(window.BOBS_CONFIG.DATA_VAULT_WEB_APP_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(body)});return new Response(JSON.stringify({ok:true,saved:true,source:'GOOGLE_SHEETS',module:body.module,outletId:body.outletId,recordKey:body.recordKey,stateVersion:v}),{status:200,headers:{'Content-Type':'application/json'}})}
