...
(function(){
  ...
  function normalizeStaff(data){if(!data||!Array.isArray(data.staff))return data;const used=new Set(data.staff.map(s=>String(s.staffId||'').trim().toUpperCase()).filter(x=>/^E\d+$/.test(x)));const out=data.staff.map(s=>{const x={...s},old=String(x.staffId||'').trim().toUpperCase();if(!/^E\d+$/.test(old)){const id=nextId(used);used.add(id);x.staffId=id;if(old)x.legacyStaffId=old;x.updatedAt=new Date().toISOString()}x.staffId=String(x.staffId||'').toUpperCase();x.monthlyAttachments=x.monthlyAttachments||{};return x});return {...data,staff:out};}
  ...
  window.fetch=function(input,init){try{const body=init&&init.body;if(typeof body==='string'&&body.indexOf('STAFF_MASTER')!==-1&&body.indexOf('moduleSave')!==-1){const p=JSON.parse(body);if(p.module===MODULE){p.outletId=MASTER_ID;p.recordKey='staff-list';if(p.data)p.data=normalizeStaff(p.data);init={...init,body:JSON.stringify(p)}}}}catch(e){}return originalFetch(input,init)};
})();

/* METHOD + WORKING DATA SAFETY GUARD */
(function(){if(window.__BOBS_WORKING_DELETE_GUARD)return;window.__BOBS_WORKING_DELETE_GUARD=true;const KEYS=new Set(['outlet-analysis-data','outlet-selection','method1-hourly-state','method2-item-state']);const nativeRemove=Storage.prototype.removeItem;Storage.prototype.removeItem=function(key){try{const k=String(key);if(KEYS.has(k)){const value=this.getItem(k);if(value&&value!=='{}'&&value!=='null')this.setItem('bobs-recovery-'+k,value);return;}}catch(e){}return nativeRemove.call(this,key);};})();
