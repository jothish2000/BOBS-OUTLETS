...
window.BOBS_CONFIG = Object.freeze({
  ...
  VERSION: '2026-09-11-google-first-v2.5'
});
...
(function(){
  ...
  async function postToVault(payload){await nativeFetch(window.BOBS_CONFIG.DATA_VAULT_WEB_APP_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(payload)});return new Response(JSON.stringify({ok:true,saved:true,source:'GOOGLE_SHEETS'}),{status:200,headers:{'Content-Type':'application/json'}})}
  async function saveBatch(input,init){let body={};try{body=JSON.parse((init&&init.body)||'{}')}catch(e){throw new Error('Invalid Google save payload')}
    if(body.action==='outletSave'||body.action==='outletUpdate'){
      const list=Array.isArray(body.outlets)?body.outlets:[];if(!list.length)return new Response(JSON.stringify({ok:false,error:'No outlets supplied'}),{status:400});
      const results=[];for(const outlet of list){const payload={action:body.action,outletId:String(outlet.id||outlet.outletId||''),outletCode:String(outlet.shortCode||outlet.code||''),outletName:String(outlet.name||outlet.outletName||''),data:Object.assign({},outlet),source:'BOBS-OUTLETS',event:'OUTLET_SETUP_SAVED',timestamp:new Date().toISOString()};if(!payload.outletId)throw new Error('Outlet ID is required');await nativeFetch(window.BOBS_CONFIG.DATA_VAULT_WEB_APP_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(payload)});results.push({outletId:payload.outletId,sent:true})}return new Response(JSON.stringify({ok:true,saved:true,source:'GOOGLE_SHEETS',count:results.length,results}),{status:200})}
    /* Legacy pages that still call their old "Sync to Google Sheet" button are
       routed into the same permanent module vault. This is a compatibility
       bridge while those pages are migrated to the Google-first UI. */
    const legacy=String(body.method||'').toLowerCase(),map={method1:'METHOD1',method2:'METHOD2',fixedexpenses:'FIXED_EXPENSES',staffroster:'STAFF_MASTER',hroutletallocation:'HR_OUTLET_ALLOCATION'};const module=map[legacy];if(module){const outletId=String(body.outletId||'COMPANY');const recordKey=String(body.recordKey||'default');return postToVault({action:'moduleSave',outletId,module,recordKey,data:body,timestamp:new Date().toISOString()})}
