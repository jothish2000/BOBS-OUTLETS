...
(function(){
  ...
  async function saveBatch(input,init){
    ...
    if(module){
      const company=(module==='STAFF_MASTER'||module==='HR_OUTLET_ALLOCATION');
      const outletId=company?'COMPANY':String(body.outletId||body.outletID||'');
      if(!outletId)throw new Error('Outlet ID is required for '+module);
      const recordKey=company?'COMPANY':String(body.recordKey||outletId);
      return postToVault({action:'moduleSave',outletId,module,recordKey,data:body,source:'BOBS-OUTLETS',event:'LEGACY_MODULE_SYNC',timestamp:body.timestamp||new Date().toISOString()})
    }
