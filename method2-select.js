...
async function load(){try{if(!outlet)throw Error('Select an outlet first.');baseline=M2.state(await M2.read(outlet));render();$('status').textContent='Outlet '+outlet+' · Selections loaded from Google.';$('retry').hidden=true}catch(e){$('status').textContent=e.message;$('retry').hidden=false}}
