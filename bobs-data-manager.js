...
(function(){
  ...
  function jsonp(action, params, timeout){
    return new Promise((resolve,reject)=>{
      if(!VAULT_URL)return reject(new Error('Google Data Vault URL is not configured'));
      const cb='bobsApi_'+Date.now()+'_'+Math.random().toString(36).slice(2);
      ...
      const timer=setTimeout(()=>finish(new Error('Google Data Vault request timed out')),timeout||12000);
      window[cb]=data=>finish(null,data);
      script.onerror=()=>finish(new Error('Unable to contact Google Data Vault'));
      const q=new URLSearchParams({action,callback:cb,t:String(Date.now()),...(params||{})});
      script.src=VAULT_URL+'?'+q.toString();
      script.async=true;
      document.head.appendChild(script);
    });
  }

  async function post(action, outlet){
    if(!VAULT_URL)throw new Error('Google Data Vault URL is not configured');
    const response=await fetch(VAULT_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action,outlet}),keepalive:true});
    return response;
  }
  ...
  async function saveToGoogle(outlet){
    ...
    for(let attempt=0;attempt<5;attempt++){
      try{
        const result=await jsonp('outletGet',{outletId:String(outlet.id)},12000);
        if(result&&result.ok&&result.found&&result.outlet){
          const g=result.outlet;
          if(String(g.outletId)===String(outlet.id))return g;
        }
        lastError='Google did not return the saved outlet yet';
      }catch(e){lastError=String(e)}
      await new Promise(r=>setTimeout(r,500*(attempt+1)));
    }
    throw new Error(lastError||'Google save verification failed');
  }

  function mergeLocal(outlet){
    const d=db(),id=String(outlet.id),old=d[id]||{};
    const merged={...old,...outlet,id,shortCode:outlet.shortCode||outlet.code||old.shortCode||old.code||'',updatedAt:now()};
    if(!merged.createdAt)merged.createdAt=now();
    d[id]=merged; saveCache(d); return merged;
  }

  async function ensureOutlet(outlet, options){
    const local=mergeLocal(outlet);
    if(options&&options.localOnly)return local;
    return await saveToGoogle(local);
  }

  async function saveMethod2(id,state){
    id=String(id||activeId()||''); if(!id)throw new Error('No outlet selected');
    const p=profile(id)||{id,createdAt:now()};
    const outlet={...p,method2:state,method2SavedAt:now(),updatedAt:now()};
    return await ensureOutlet(outlet);
  }

  async function recoverFromGoogle(onDone){
    try{
      const result=await jsonp('outletList',{},12000);
      if(!result||!result.ok)throw new Error((result&&result.error)||'Google returned an invalid outlet list');
      const d={};
      (result.outlets||[]).forEach(item=>{
        const o=item.data||{}; const id=String(item.outletId||o.id||''); if(!id)return;
        d[id]={...o,id,shortCode:o.shortCode||o.code||item.outletCode||'',name:o.name||item.outletName||'',createdAt:item.createdAt||o.createdAt||now(),updatedAt:item.updatedAt||o.updatedAt||now()};
      });
      saveCache(d);
      if(onDone)onDone(true,(result.outlets||[]).length,'Google OUTLET_MASTER loaded');
  ...
  window.BOBSDataManager={
    ...
    legacyOutletMasterKey:LEGACY_OUTLET_MASTER,
    ensureOutlet,
    saveMethod2,
    saveToGoogle,
    getOutlet:profile,
