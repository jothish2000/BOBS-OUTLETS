...
(function(){
  ...
  function normalizeOutlet(item){const o=item&&item.record?item.record:(item&&item.data?item.data:item||{});const id=String(item&&item.outletId!=null?item.outletId:(o.id||''));if(!id)return null;return {...o,id,shortCode:o.shortCode||o.code||(item&&item.shortCode)||((item&&item.outletCode)||''),name:o.name||(item&&item.outletName)||'',createdAt:(item&&item.createdAt)||o.createdAt||now(),updatedAt:(item&&item.updatedAt)||o.updatedAt||now(),numShifts:o.numShifts||(item&&item.numShifts)||2,shiftTimes:o.shiftTimes||(item&&item.shiftTimes)||[]};}
  ...
  function snapshotOutlets(data){const found=[];if(!data||typeof data!=='object')return found;const direct=data[OUTLET_MASTER]||data[LEGACY_OUTLET_MASTER];if(direct&&typeof direct==='object'&&!Array.isArray(direct))Object.keys(direct).forEach(id=>{const o=normalizeOutlet(direct[id]);if(o)found.push(o)});const sel=data[ACTIVE_OUTLET];const candidates=Array.isArray(sel)?sel:[sel];candidates.forEach(x=>{if(!x||typeof x!=='object')return;const o=normalizeOutlet(x);if(o)found.push(o)});const analysis=data['outlet-analysis-data'];if(Array.isArray(analysis))analysis.forEach(x=>{if(!x||typeof x!=='object')return;const o=normalizeOutlet(x);if(o)found.push(o)});else if(analysis&&typeof analysis==='object')Object.keys(analysis).forEach(id=>{const o=normalizeOutlet({...analysis[id],id:analysis[id]&&analysis[id].id||id});if(o)found.push(o)});return found;}
  async function recoverProtectedOutlets(){const listed=await jsonp('list',{},12000);if(!listed||!listed.ok||!Array.isArray(listed.snapshots))return [];const snaps=listed.snapshots.slice().sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));const found={};for(const s of snaps){if(!s||!s.snapshotId)continue;try{const r=await jsonp('restore',{snapshotId:s.snapshotId},12000);if(!r||!r.ok)continue;for(const o of snapshotOutlets(r.data||{})){if(o&&o.id&&!found[o.id])found[o.id]=o}}catch(e){}if(Object.keys(found).length)break;}const arr=Object.values(found);if(arr.length){const d=db();arr.forEach(o=>d[o.id]={...(d[o.id]||{}),...o});saveCache(d)}return arr;}
  async function recoverLegacyOutletMaster(){try{const result=await jsonp('outletList',{},12000);if(result&&result.ok&&Array.isArray(result.outlets)&&result.outlets.length)return cacheOutlets(result.outlets);}catch(e){}return [];}
  ...
  async function recoverFromGoogle(onDone){try{const result=await jsonp('listOutletMaster',{},12000);if(!result||!result.ok)throw new Error((result&&result.error)||'Google returned an invalid outlet master');const d={};(result.outlets||[]).forEach(item=>{const o=normalizeOutlet(item);if(o)d[o.id]=o});if(Object.keys(d).length){saveCache(d);if(onDone)onDone(true,Object.keys(d).length,'Google OUTLET_MASTER loaded');return Object.values(d)}const legacy=await recoverLegacyOutletMaster();if(legacy.length){if(onDone)onDone(true,legacy.length,'Recovered existing outlets from legacy Google OUTLET_MASTER');return legacy}const recovered=await recoverProtectedOutlets();if(recovered.length){if(onDone)onDone(true,recovered.length,'Recovered protected outlet profile from BOBS Data Vault snapshot');return recovered}saveCache({});if(onDone)onDone(true,0,'Google OUTLET_MASTER contains no permanent outlets');return [];}catch(e){const legacy=await recoverLegacyOutletMaster();if(legacy.length){if(onDone)onDone(true,legacy.length,'Recovered existing outlets from legacy Google OUTLET_MASTER');return legacy}try{const recovered=await recoverProtectedOutlets();if(recovered.length){if(onDone)onDone(true,recovered.length,'Recovered protected outlet profile from BOBS Data Vault snapshot');return recovered}}catch(x){}if(onDone)onDone(false,0,String(e));throw e;}}
