...
(async()=>{
 ...
 assert.equal(await editor.locator('#packingPer').inputValue(),'2');
 ...
 await editor.getByLabel('Price ₹ each',{exact:true}).nth(1).fill('3');
 ...
 const key=cat+'::0',savedItem=clone(db['1/METHOD2/default'].itemEditors[key]);assert.equal(savedItem.packingPer,'2');assert.equal(db['1/METHOD2/default'].packaging[key][0].qty,.5);assert.equal(db['1/METHOD2/default'].commercial[key].totalSoldCogs,2400);
 ...
 const firstScreen=page.waitForEvent('popup');await page.locator('#method2Btn').click();const menu=await firstScreen;await menu.waitForSelector('.category-tile');assert.equal(await menu.locator('.category-tile').count(),13);
