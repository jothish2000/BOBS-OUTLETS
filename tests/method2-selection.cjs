...
(async()=>{
 ...
 const key=cat+'::0',savedItem=clone(db['1/METHOD2/default'].itemEditors[key]);assert.equal(Number(savedItem.packingPer),2);assert.equal(savedItem.condiments[0].packingMode,'required');assert.equal(db['1/METHOD2/default'].commercial[key].totalPackingCost,1200);assert.equal(db['1/METHOD2/default'].packaging[key][0].qty,.5);assert.equal(db['1/METHOD2/default'].commercial[key].totalSoldCogs,2400);
