...
const cfg={
 'method1.html':{module:'METHOD1',key:'method1-hourly-state',label:'Method 1'},
 'method2.html':{module:'METHOD2',key:'method2-item-state',label:'Method 2'},
 'staff.html':{module:'STAFF',key:'staff-data',label:'Staff / HR'},
 'fixed-expenses.html':{module:'FIXED_EXPENSES',key:'fixed-expenses-data',label:'Fixed Expenses'}
}[path];
if(!cfg)return;
...
function recordsPayload(r){if(!r)return null;let d=r.data;if(typeof d==='string'){try{d=JSON.parse(d)}catch(e){}}return d}
...
function show(data){
 ...
 box.querySelector('.bobs-entry-fresh').onclick=()=>{try{localStorage.removeItem(cfg.key)}catch(e){}ov.remove();window.dispatchEvent(new Event('bobs-start-fresh'))};
}
async function init(){
 let g=await googleState();
 if(g){show(g);return}
 if(localHas() && path!=='method1.html'){show({data:JSON.parse(localStorage.getItem(cfg.key))});return}
