/* BOBS Business Forecast & Scenario Engine v1.2 — pure planning calculations, no writes */
...
function forecast(i){
 ...
 const sales=s*vol*price+safe(i.eventExtraDailyRevenue)+safe(i.bulkRevenueDaily),gp=sales-cogs*vol,grossMargin=sales?gp/sales:0,contribution=sales-cogs*vol-variable,cr=sales?contribution/sales:0;
 const pbdit=gp-staff-fixed-variable,pbit=pbdit-dep,pbt=pbit-finance,tax=Math.max(0,pbt*pct(i.taxRate)),net=pbt-tax;
 const be=cr>0?(staff+fixed+dep+finance)/cr:Infinity;
 const target=n(i.targetPbdit),den=1-(s?(c/s):0)-(s?(v/s):0),req=den>0?(staff+fixed+target)/den:Infinity;
 return{sales,cogs:cogs*vol,grossProfit:gp,grossMargin,staff,fixed,variable,depreciation:dep,finance,pbdit,pbit,pbt,tax,netProfit:net,contribution,contributionRatio:cr,breakEvenRevenue:be,breakEvenGap:safe(be-sales),requiredSalesForPBDIT:req,requiredAdditionalSalesForPBDIT:safe(req-sales)};
}
function period(r,days){const d=safe(days);return{days:d,sales:r.sales*d,cogs:r.cogs*d,grossProfit:r.grossProfit*d,staff:r.staff*d,fixed:r.fixed*d,variable:r.variable*d,depreciation:r.depreciation*d,finance:r.finance*d,pbdit:r.pbdit*d,pbit:r.pbit*d,pbt:r.pbt*d,tax:r.tax*d,netProfit:r.netProfit*d}}
...
global.BOBSForecastEngine={version:'1.2',forecast,period,sensitivity,eventScenario,bulkScenario,capacity};
})(window);
