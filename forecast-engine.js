/* BOBS Business Forecast & Scenario Engine v1.4 — pure planning calculations, no writes */
...
function forecast(i){
 ...
 const cogsFactor=Math.max(0,1+pct(i.cogsChangePct)),staff=st*Math.max(0,1+pct(i.staffCostChangePct)),fixed=f*Math.max(0,1+pct(i.fixedExpenseChangePct)),variable=v*vol*Math.max(0,1+pct(i.variableExpenseChangePct));
 const dep=d*Math.max(0,1+pct(i.depreciationChangePct)),finance=fin*Math.max(0,1+pct(i.financeCostChangePct));
 const eventFactor=Math.max(0,1+pct(i.eventSalesUpliftPct));
 const sales=s*vol*price*eventFactor+safe(i.eventExtraDailyRevenue)+safe(i.bulkRevenueDaily);
 const cogsValue=c*vol*eventFactor*cogsFactor;
 ...
 const target=n(i.targetPbdit),req=cr>0?(staff+fixed+bulkExtra+target)/cr:Infinity;
 return{sales,cogs:cogsValue,grossProfit:gp,grossMargin,staff,fixed,variable,bulkExtraCost:bulkExtra,depreciation:dep,finance,pbdit,pbit,pbt,tax,netProfit:net,contribution,contributionRatio:cr,breakEvenRevenue:be,breakEvenGap:be===Infinity?Infinity:be-sales,requiredSalesForPBDIT:req,requiredAdditionalSalesForPBDIT:safe(req-sales)};
}
...
global.BOBSForecastEngine={version:'1.4',forecast,period,sensitivity,eventScenario,bulkScenario,capacity};
})(window);
