...
function show(id){active=id;baseline=clone(find(master,id));$('yield').value=baseline.yieldQty||baseline.standardYield||baseline.yield||'';$('yieldUnit').value=baseline.yieldUnit||'';$('lines').replaceChildren();(baseline.ingredients||[]).forEach(addLine);$('editor').hidden=false;dirty=false;calculate();status('Loaded '+baseline.name+' from Google. Edits are unsaved until verified.')}
...
$('reload').onclick=async()=>{if(dirty&&!confirm('Discard unsaved edits and reload from Google?'))return;try{master=await read();show(active)}catch(e){status(e.message)}};
...
try{status('Checking latest Google record…');const latest=await read(),current=find(latest,active);if(!same(current,baseline))throw Error('This recipe changed elsewhere. Reload it before saving to avoid overwriting newer rates.');
const updated=Object.assign({},current,{ingredients:values,yieldQty:y,yieldUnit:$('yieldUnit').value.trim(),batchCost:total,totalCost:total,unitCost:total/y,costPerUnit:total/y,perUnitCost:total/y,productionCost:total/y,costUpdatedAt:new Date().toISOString()});
for(const k of ['standardYield','yield','standardYieldQty'])if(k in updated)updated[k]=y;
latest.recipes=latest.recipes.map(r=>identity(r)===active?updated:r);
