...
W.plan=(profiles,positions=[])=>{const tasks=[],warnings=[];for(const p of W.consolidate(profiles)){const start=W.mins(p.start);if(start===null||!p.role||!p.qty||!p.capacity){warnings.push((p.name||'Item')+': incomplete time/role/quantity/capacity');continue}tasks.push({name:p.name,role:p.role,start,end:start+p.elapsedMinutes,activeMinutes:p.activeMinutes,equipmentMinutes:p.equipmentMinutes,elapsedMinutes:p.elapsedMinutes,equipment:p.equipment||'',cycles:p.cycles,position:p.position||''})}
