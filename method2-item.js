...
function calculate(){
 ...
 lines('pricing',[['Original catalogue price',money(M2.basePrice(baseline,cat,i,item))],['Current selling price (editable above)',money(M2.number(d.price))],['Food spoilage applied',c.spoilageApplied?'YES':'NO'],['Food spoilage pricing allowance',c.spoilageApplied?d.spoilage+'%':'0%'],['COGS after food spoilage',incomplete?'Incomplete':money(c.final)],['UUWP applied',c.apply?'YES':'NO'],['UUWP pricing allowance',c.apply?d.uuwp+'%':'0%'],['Pricing COGS after UUWP',incomplete?'Incomplete':money(c.withUuwp)],['Suggested price with '+(d.pricingBasis==='margin'?'target margin':'markup'),incomplete?'Incomplete':money(c.suggested)],['Current vs suggested',incomplete||M2.number(d.price)===null?'Incomplete':((M2.number(d.price)-c.suggested)>=0?'+':'')+money(M2.number(d.price)-c.suggested)]]);
