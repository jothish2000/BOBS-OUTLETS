...
(function(){'use strict';
  ...
  function sync(el){
    ...
    if(uv!==null)o.unitsPerBatch=uv;
    if(bv!==null)o.batchesToday=bv;
    ...
    if(panel){
      const today=panel.querySelector('.m2v4-today-v');
  ...
  function install(){
    document.addEventListener('input',function(e){
      ...
      if(t.classList.contains('batchSizeInput')||t.classList.contains('numBatchesInput')||t.classList.contains('maxBatchesPerDayInput')){
        /* Stop the old runtime's per-keystroke re-render. The field must stay focused/editable. */
        e.stopPropagation();
    ...
    document.addEventListener('change',function(e){
      const t=e.target; if(!t||!t.classList) return;
      if(t.classList.contains('batchSizeInput')){
        const row=t.closest('.batchRow'); if(row){const s=row.querySelector('span'); if(s) s.innerHTML='<b>No. of pieces per batch</b>';}
      }
    },true);
    function label(){
      document.querySelectorAll('.batchSizeRow .batchSizeInput').forEach(function(t){
        t.readOnly=false; t.disabled=false; t.style.pointerEvents='auto';
        const r=t.closest('.batchRow'); if(r){const s=r.querySelector('span'); if(s) s.innerHTML='<b>No. of pieces per batch</b>';}
      });
    }
